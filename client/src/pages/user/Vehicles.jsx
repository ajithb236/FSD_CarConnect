...
// Hardcoded vehicles with asset images
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!res.ok) {
          console.log("API not available, using hardcoded vehicles");
          dispatch(showVehicles(HARDCODED_VEHICLES));
          setIsLoading(false);
          return;
        }
        if (res.ok) {
        ...
        console.log("Error fetching vehicles, using hardcoded data:", error);
        dispatch(showVehicles(HARDCODED_VEHICLES));
        setIsLoading(false)             
      }
    };
